...
| Parser | Gaussian 完整/失败解析 | PASS | 2.4 | Gaussian parser 覆盖正常终止、Gibbs、虚频、HOMO/LUMO、NBO 与失败中文提示。 |
...
| API | 核心量子化学与自由基接口 | PASS | 218.7 | 核心 API、科学计算任务矩阵、能量工作台与 BDE 入口正常响应且保持中文 provenance/mock 边界。 |
| API | 错误路径与中文提示 | PASS | 6.7 | 缺失能量、空 log、非法 cube 均返回中文错误或 failed 质量评分。 |
| API | GoodVibes/QTAIM/NCI/MCP 安全扩展 | PASS | 24.8 | 只读 parser 与 MCP 白名单工作流可追踪、可测试。 |
| 数据库 | 已实现表与提示要求表覆盖 | PASS | 0.3 | 数据库表覆盖完整。 |
| 安全 | 上传路径与文件类型边界 | PASS | 7.6 | 上传白名单、路径穿越和仅读取边界通过。 |
