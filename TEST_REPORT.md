...
## 2026-06-01 增量：真实科学计算软件连接守卫

- 新增 `check-version`、`dry-run`、`confirm`、`execute` API 路径；默认全部不运行外部科学计算程序。
- `dry-run` 返回命令模板、预期输出和拒绝原因，不执行 Gaussian/cubegen/Multiwfn/GoodVibes。
- `execute` 在未设置 `ENABLE_REAL_QC_EXECUTION=1`、缺少 `confirmed_execute` 或缺少用户二次确认时返回中文拦截信息。
- MCP 新增 `validate_external_tool`、`dry_run_simulation_job`、`confirm_simulation_job`、`execute_confirmed_simulation_job`，用于 Agent 工作流测试安全拒绝路径。
- 新增 `docs/REAL_QC_SOFTWARE_INTEGRATION.md` 和 `docs/EXTERNAL_EXECUTION_SECURITY.md`。
...
| API | 核心量子化学与自由基接口 | PASS | 166.9 | 核心 API、科学计算任务矩阵、能量工作台与 BDE 入口正常响应且保持中文 provenance/mock 边界。 |
| API | 错误路径与中文提示 | PASS | 7.2 | 缺失能量、空 log、非法 cube 均返回中文错误或 failed 质量评分。 |
| API | GoodVibes/QTAIM/NCI/MCP 安全扩展 | PASS | 25.2 | 只读 parser 与 MCP 白名单工作流可追踪、可测试。 |
| 数据库 | 已实现表与提示要求表覆盖 | PASS | 0.3 | 数据库表覆盖完整。 |
| 安全 | 上传路径与文件类型边界 | PASS | 7.3 | 上传白名单、路径穿越和仅读取边界通过。 |
