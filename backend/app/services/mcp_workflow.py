...
SAFE_TOOLS = [
    ...
    {"name": "calculate_bde_roor", "title": "计算 RO–OR BDE", "title_zh": "计算 RO–OR BDE", "description": "按片段和母体 Gibbs 能计算过氧键 BDE。", "description_zh": "按片段和母体 Gibbs 能计算过氧键 BDE。", "input_schema": {"g_fragments_hartree": "float", "g_molecule_hartree": "float"}, "output_schema": {"bde_kcal_mol": "float"}, "execution_mode": "calculation", "can_execute_external": False, "required_confirmation": False, "safety_boundary": "只运行内置公式。"},
    ...
    {"name": "audit_scientific_formulas", "title": "审计科学公式契约", "title_zh": "审计科学公式契约", "description": "返回固定常数、公式、数值稳定策略和证据边界。", "description_zh": "返回固定常数、公式、数值稳定策略和证据边界。", "input_schema": {}, "output_schema": {"constants": "dict", "formulas": "list", "evidence_policy": "dict"}, "execution_mode": "audit_only", "can_execute_external": False, "required_confirmation": False, "safety_boundary": "只读取内置数理契约，不执行外部程序。"},
    {"name": "inspect_external_tool_configuration", "title": "检查外部软件配置", "title_zh": "检查外部软件配置", "description": "只检查已登记路径和环境变量，不执行 version command。", "description_zh": "只检查已登记路径和环境变量，不执行 version command。", "input_schema": {"tool_type": "gaussian16|formchk|cubegen|multiwfn|goodvibes|slurm"}, "output_schema": {"configured": "bool", "path_exists": "bool", "status": "str"}, "execution_mode": "inspection_only", "can_execute_external": False, "required_confirmation": False, "safety_boundary": "不执行外部软件或版本命令。"},
    ...
    {"name": "generate_formchk_template", "title": "生成 formchk 模板", "title_zh": "生成 formchk 模板", "description": "生成 formchk chk->fchk 命令模板，不执行。", "description_zh": "生成 formchk chk->fchk 命令模板，不执行。", "input_schema": {"input_chk": "str"}, "output_schema": {"command_template": "str"}, "execution_mode": "template_only", "can_execute_external": False, "required_confirmation": False, "safety_boundary": "不执行 formchk。"},
    ...
    {"name": "generate_multiwfn_esp_template", "title": "生成 Multiwfn ESP 模板", "title_zh": "生成 Multiwfn ESP 模板", "description": "生成 Multiwfn ESP extrema 脚本模板，不执行。", "description_zh": "生成 Multiwfn ESP extrema 脚本模板，不执行。", "input_schema": {"input_file": "str"}, "output_schema": {"script": "str", "command_template": "str"}, "execution_mode": "template_only", "can_execute_external": False, "required_confirmation": False, "safety_boundary": "不执行 Multiwfn。"},
    ...
    {"name": "generate_goodvibes_template", "title": "生成 GoodVibes 模板", "title_zh": "生成 GoodVibes 模板", "description": "生成 GoodVibes 热化学校正命令模板，不执行。", "description_zh": "生成 GoodVibes 热化学校正命令模板，不执行。", "input_schema": {"pattern": "str"}, "output_schema": {"command_template": "str"}, "execution_mode": "template_only", "can_execute_external": False, "required_confirmation": False, "safety_boundary": "不执行 GoodVibes。"},
    ...
    {"name": "validate_external_tool", "title": "校验外部科学计算工具", "title_zh": "校验外部科学计算工具", "description": "校验工具路径和模式，不执行 version command。", "description_zh": "校验工具路径和模式，不执行 version command。", "input_schema": {"tool_type": "str", "executable_path": "str|null"}, "output_schema": {"validation_status": "str", "can_execute": "bool"}, "execution_mode": "validation_only", "can_execute_external": False, "required_confirmation": False, "safety_boundary": "只做路径格式校验，不执行外部程序。"},
    {"name": "dry_run_simulation_job", "title": "科学计算任务 dry-run", "title_zh": "科学计算任务 dry-run", "description": "返回任务执行计划和拒绝/放行条件，不执行外部程序。", "description_zh": "返回任务执行计划和拒绝/放行条件，不执行外部程序。", "input_schema": {"job_id": "int"}, "output_schema": {"status": "dry-run", "will_execute": "false"}, "execution_mode": "dry_run", "can_execute_external": False, "required_confirmation": False, "safety_boundary": "dry-run 不运行 Gaussian/cubegen/Multiwfn。"},
    {"name": "confirm_simulation_job", "title": "确认科学计算任务", "title_zh": "确认科学计算任务", "description": "记录用户二次确认，但不执行任务。", "description_zh": "记录用户二次确认，但不执行任务。", "input_schema": {"job_id": "int", "user_confirmed": "bool", "confirmation_phrase": "str"}, "output_schema": {"confirmed": "bool"}, "execution_mode": "confirmation_only", "can_execute_external": False, "required_confirmation": True, "safety_boundary": "只记录确认状态，不执行外部程序。"},
    {"name": "execute_confirmed_simulation_job", "title": "执行已确认科学计算任务", "title_zh": "执行已确认科学计算任务", "description": "默认被安全守卫拦截；仅用于测试拒绝路径和未来外部 runner。", "description_zh": "默认被安全守卫拦截；仅用于测试拒绝路径和未来外部 runner。", "input_schema": {"job_id": "int", "user_confirmed": "bool"}, "output_schema": {"status": "blocked|ready-but-not-run"}, "execution_mode": "guarded_execute", "can_execute_external": False, "required_confirmation": True, "safety_boundary": "默认不执行；需要 ENABLE_REAL_QC_EXECUTION=1、confirmed_execute 和独立 runner。"},
    {
